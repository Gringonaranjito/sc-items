...
function buyEntryStatsGroups(entry, tab = state.buyTab) {
  ...
  if (tab === "ships" || tab === "rentals") {
    ...
    const dims = [entry?.length ? `L ${cleanDisplayText(entry.length)}` : "", entry?.width ? `W ${cleanDisplayText(entry.width)}` : "", entry?.height ? `H ${cleanDisplayText(entry.height)}` : "", entry?.mass ? `Mass ${cleanDisplayText(entry.mass)}` : ""].filter(Boolean).join(" Â· ");
...
function scminersDbEntrySummary(entry) {
  ...
  return summaryBits.filter(Boolean).join(" Â· ");
}
...
function scminersDbPrereqSummary(entry) {
  ...
  for (const block of [...blocks, ...prereqs]) {
    ...
    if (blockParts.length) parts.push(blockParts.join(" Â· "));
  }
  ...
  const fallbackBits = [
    ...
    .map((value) => cleanDisplayText(value))
    .filter(Boolean);
  if (!parts.length && fallbackBits.length) return fallbackBits.join(" Â· ");
  return parts.join(" | ");
}
    ...
    .map(
      (entry) => `
        <div class="mission-line">
          ...
          <div class="muted">${formatCount(entry.quantity || entry.quantity_required || 0)} SCU${entry.minQuality !== undefined || entry.min_quality !== undefined ? ` Â· min quality ${formatCount(entry.minQuality ?? entry.min_quality ?? 0)}` : ""}</div>
        </div>
      `,
    )
    .join("");
    ...
    .map((entry) => {
      ...
      return `
        <div class="mission-line">
          ...
          <div class="muted">${totalReturned ? `${formatCount(totalReturned)} SCU returned` : "Return quantity unavailable"}${entry?.recipe_time_seconds ? ` Â· ${formatCount(entry.recipe_time_seconds)} sec` : ""}</div>
          ${resultMarkup}
...
function renderSummary() {
  ...
  els.footerWatch.textContent = footerBits.join(" Â· ");
}
...
function renderSelected() {
  ...
  els.selectedMeta.textContent = `${mission.type || "Mission"} Â· ${mission.faction || "Unknown"} Â· ${mission.repStanding || "Any rank"}`;
  ...
  els.selectedDetails.innerHTML = `
    ...
    <div class="mission-section">
      <div class="mission-section-head">
        <span>Mission Description</span>
        <span>${formatCount(mission.repReward || 0)} points Â· ${moneyRewardLabel}${scriptReward ? ` Â· ${formatCount(scriptReward)} script` : ""}</span>
      </div>
    ...
    <div class="mission-section">
      ...
      <div class="mission-summary-line">Rewards left: ${formatCount(missingRewards.length)} Â· Points: ${formatCount(mission.repReward || 0)} Â· Money: ${moneyRewardLabel}${scriptReward ? ` Â· Script: ${formatCount(scriptReward)}` : ""}</div>
  ...
  els.missionSearchResults.innerHTML = `
      ...
      <div class="timeline">
        ...
        .map((mission) => {
          ...
          const moneyLabel = ` Â· ${missionMoneyRewardLabel(mission)}`;
          const scriptLabel = scriptReward ? ` Â· ${formatCount(scriptReward)} script` : "";
    ...
    <div class="detail-grid buy-detail-grid">
      ...
      ${((tab === "ships" || tab === "rentals") && (selected.length || selected.width || selected.height || selected.mass)) ? `
        <div class="detail-kv buy-detail-wide">
          ...
          <strong>${[selected.length ? `L ${selected.length}` : "", selected.width ? `W ${selected.width}` : "", selected.height ? `H ${selected.height}` : "", selected.mass ? `Mass ${selected.mass}` : ""].filter(Boolean).join(" Â· ")}</strong>
        </div>
      ` : ""}
      ${tab === "items" ? `
        <div class="detail-kv buy-detail-wide">
      ...
      ${tab === "ships" || tab === "rentals" ? selectedShipWeapons.length ? `<div class="detail-kv buy-detail-wide"><span>Weapons</span><strong>${weaponCounts.map(([name, count]) => `${name} x ${count}`).join(" Â· ")}</strong></div>` : "" : ""}
      ${tab === "ships" || tab === "rentals" ? selectedShipComponents.length ? `<div class="detail-kv buy-detail-wide"><span>Components</span><strong>${componentCounts.map(([name, count]) => `${name} x ${count}`).join(" Â· ")}</strong></div>` : "" : ""}
        ...
        .map((entry, index) => {
          ...
          return `
            <div class="mission-line">
              ...
              <div class="muted">${escapeHtml(type || "Unknown type")}${summary ? ` Â· ${escapeHtml(summary)}` : ""}</div>
            </div>
          `;
        })
        .join("");
