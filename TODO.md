...
8. Fix the Parallax "Sunstone" Energy Assault Rifle blueprint so it is clickable in the blueprint section.
9. [done] Add mission unlock guidance so the app can show how to make a mission appear, including prerequisites and rank or faction requirements.
10. Add rank progress details so the app can show how many points and missions are needed to reach each rank in each faction or company.
11. [done] Add a remove action for owned blueprints so accidental collection entries can be undone.
12. [done] Add a manual "add blueprint" action so an already-owned blueprint can be added to a user's collection.
13. Add tablet/mobile-friendly layouts for the app.
14. Add a phone/tablet companion app with export/import or sync between devices.
15. Package the project for GitHub Pages/downloads and add install/use instructions.
16. Add a shortcut launcher with the app logo so it opens without index.html.
